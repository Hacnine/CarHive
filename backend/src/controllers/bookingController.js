...
const getUserBookings = async (req, res) => {
  try {
    ...
    if (status) {
      // Handle multiple ways status can be passed:
      // - comma separated string: ?status=confirmed,reserved
      // - repeated params (array): ?status=confirmed&status=reserved
      let statusArray = [];
      if (Array.isArray(status)) {
        statusArray = status.map(s => String(s).trim());
